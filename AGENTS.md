...
- UI strings: wrap user-visible text in `tr()` from `ui/i18n.py` with named placeholders (`tr("Connecting to {url} ...", url=url)`), never positional `{0}`. Label tables defined at module level are marked with `N_("...")` and passed through `tr()` where they are rendered (module-level `tr()` would run before the language is set). English constants that live in `core/` and reach the screen (`CHECK_LABELS`, `CHANGE_KIND_LABELS`, `EVALUATION_LABELS`, `BACKEND_LABELS`, `FALLBACK_EXPLANATIONS`, ...) stay English in core and are translated at the UI boundary (`tr(CHECK_LABELS[check])`; `ui.i18n.core_source_strings` lists them for extraction). Identifiers that the logic compares (`PROMPTS` keys, `chapter_mode`, explanation language keys) keep a display-name mapping (`ui/app.py: MODE_LABELS`, `ui/workflow_screen.py: LANGUAGE_LABELS`). Numbers shown to the user go through `ui.i18n.format_number`. Backend error messages stay English inside exceptions; the UI prefixes them with a translated sentence. `python scripts/extract_strings.py de` lists strings missing from `locales/de.json` (`--update` adds empty keys) and `tests/test_i18n.py` fails when a source string has no German entry or the placeholders differ, so every new string needs its translation in the same change. The language comes from `AppSettings.ui_language` (`auto|en|de`, env `TEAI_LANG`) and is applied once at start-up, so changes need a restart.
