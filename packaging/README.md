# Packaging TextEnhanceAI for Windows and macOS

Authors who do not run `pip` get a folder (Windows) or an app bundle (macOS)
...
`windows-latest` and `macos-latest` whenever a tag `v*` is pushed and attaches
the results to the GitHub release. The version shown in the window title,
the About dialog and the bundle comes from `core/__init__.py` (`__version__`).
...
python packaging/make_icon.py                      # icon.ico / icon.icns from icon.svg
...
- Linux: `dist/TextEnhanceAI/TextEnhanceAI` (not shipped by the workflow, but
  the spec works there too).
...
The workflow builds both platforms, runs the test suite on Ubuntu, and
uploads `TextEnhanceAI-v0.14-windows.zip` and `TextEnhanceAI-v0.14-macos.dmg`
to the release for that tag. The Help menu's *Releases on GitHub* entry opens
the releases page; the app does not update itself.

## Notes and limitations

- **Unsigned.** Neither build is code-signed or notarised. Windows shows the
